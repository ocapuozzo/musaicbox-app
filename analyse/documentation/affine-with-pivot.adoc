...
Focus on inversion with formula: stem:[c cdot (ax + b) |->]   stem:[c cdot (ax + p(1 - a) + k)]

.AffinePivot analysis : {startsb}C]T~k~Ma~p~(A)
[%header,cols=".^,.^,.^,.^,.^,.^,.^2"]
|===
^a|stem:[c] ^a|stem:[p] ^a|stem:[a] ^a|stem:[k] .>a|stem:[c cdot (ax + p(1 - a) + k)]|_{startsb}C]T~k~Ma~p~(A)_ |designation
//
// |no present
// |no present
// |1
...
In the case of the inversion operation, David Lewin demonstrated <<LEWIN-1977a>> that one could freely define an axis of symmetry through a pair (a,b), _I^a,b^_, historically called _I^uv^_, but we will use _a,b_ with comma separator for greater readability and to emphasize the concept of a line segment. Here is Lewin's equivalent version to the affine version with pivot of the example above.
...
Later (1977), D. Lewin introduced the inversion operator stem:[I^(uv)], parameterized  by a pair of pitch-classes.

The inversion operator stem:[I^(a, b)] (<<LEWIN-1977a>>, <<LEWIN-1980>>)
...
[.text-center]
Ex. 1: A-type axis with pairs: (2,2), (1,3)... and Ex. 2: B-type axis with pairs: (0,1), (11,2)...

This works for axes of symmetry of type A and B. Indeed, if a = b, then the axis will pass through stem:[(2a)/2 = a], and Lewin suggests denoting such segments stem:[(a,a)], for an axis of type A, simply by stem:[a], which is then synonymous with _p_ (pivot value):
...
* When _k_ is odd, the midpoint is stem:[ p + k/2 = p + (k-1)/2 + 1/2], +
...
_m_ = stem:[p + k/2]{nbsp}{nbsp}{nbsp}or{nbsp}{nbsp}{nbsp}stem:[p + (k-1)/2 + 1/2]
...
Others expressions with valid p-value : +
 `T~3~M11~5~([256])` = `T~1~M11~6~([256])` = `T~-3~M11~2~([256])` -> `[7811]`
