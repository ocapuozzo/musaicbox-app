...
=== Simplified notation of the affine expression
...
.AffinePivot analysis : {startsb}C]T~k~Ma~p~(A)
[%header,cols=".^,.^,.^,.^,.^,.^,.^2"]
|===
^a|stem:[c] ^a|stem:[p] ^a|stem:[a] ^a|stem:[k] .>a|stem:[c cdot (ax + p(1 - a) + k)]|{startsb}C]T~k~Ma~p~ |designation
...
.Two symmetry axis types (n=8) A through 0, B between 0 and 1
image::axis-symmetry-n8.png[]

Note that when n is odd, there is only one type of axis that is both intercalary [.underline]#and# median.
...
=== More than one bowstring to his axis.

A type *_B_* axis passes through the midpoint of the segment defined by two pitch-class numbers.

For example, see below axis passing through pitch-class numbers *_2_* and *_3_*.

image:pcs-24567-axe-B.png[] `??` -> image:pcs-0131011-axe-B.png[]

To solve this case, let's take a look at D. Lewin's inversion operator stem:[I^(u v)] (<<LEWIN-1980>>).

This operator allows you to specify a pivot symmetry axis for inversion, by designating a pair stem:[(a,b)] of pitch-class numbersfootnote:[Lewin most often uses the names _u_ and _v_ to denote this pair. We us _ab_ for to emphasize the notion of linear segment].
...
NOTE: The line perpendicular to segment (a,b) passing through its midpoint, through the circle's center, forming a symmetry axis.


This works for axes of symmetry of type A and B. Indeed, if a = b, then the axis will pass through stem:[a], from stem:[(2a)/2 = a], and Lewin suggests denoting such segments stem:[(a,a)], for an axis of type A, simply by stem:[a], which is then synonymous with _p_ (pivot value):

[.text-center]
stem:[I^(a,a) |-> I^a]


In the given example, the B-type axis passes between two pitch-class numbers, 2 and 3. Therefore, a solution to the problem posed is: `I^2,3^`

[.text-center]
`I^2,3^([2 4 5 6 7]) = [0 1 3 10 11]`


There are multiple possible expressions of (_a,b_) to specify a same axis: (_a,b_) and their inverse (_b,a_).

[caption=""]
.Example: 6 line segments perpendicular of a symmetry axis
====
image:pcs-24567-axe-segments.png[]{nbsp}{nbsp} stem:[I^(a,b) |->  I^(2,3) = I^(1,4) ... = I^(9,8)] and their inverse stem:[I^(3,2) = I^(4,1)] etc.
====

TIP: With _n = 12_ there are 6  line segments perpendicular of a symmetry axis, one of which is self-inverse (triton 6,6). We therefore have 5*12 + 6 = 66  different pairs (a,b) + _n_ "self  pair" = 78 cases. +
 66 is the number of 2-combinations in 12, so:  stem:[((12),(2)) = (12!) / (2(10!)) = 66]
...
We know that expression stem:[T_iI^a] is denoted stem:[T_iM11_a] in affine pivot system version ("_a_" designated the pivot-value "_p_" for A-type symmetry axis).

[IMPORTANT]
====

From equation stem:[I^(p,p+k) = T_kM11_p], It becomes clear that role of _k_ is to assign a length to the segment [.nowrap]#(_p, p + k_)#. It is this length which makes it possible to identify the axis of symmetry of the function.
...
* When _k_ = _0_, the midpoint is stem:[p + 0 = p], thefore, the operation stem:[T_0M11_p] denote an inversion around an axis passing through _p_ (axis of type A)

* When _k_ is odd, the midpoint is stem:[ p + k/2 = p + 1/2 + (k-1)/2 = 1/2 + (2p - 1 + k ) /2], +
Since _(2p - 1)_ and _k_ have the same parity (odd), the value of _(2p - 1 + k)_ is always even, so an odd _k_ always denotes a type B axis.

* When _k_ is even, the midpoint is stem:[p + k/2] always denote an even value, so an axis of type A.

TIP: When _k_ is even, the axis is type of A, otherwise the axis is of type B.

Examples of median (A) and inter (B) midpoint axis at{nbsp}stem:[p + k/2] :

|===
^|(k even) +
midpoint = stem:[p + k/2] ^|(k odd) +
midpoint = stem:[p + k/2]{nbsp}{nbsp}{nbsp}or{nbsp}{nbsp}{nbsp}stem:[1/2 + (2p - 1 + k ) /2]
...
^a|`T~0~M11~2~([2]) -> [2]` {nbsp} {nbsp} midpoint: stem:[ 2 + 0/2 = 2]
^a|`T~1~M11~2~([2]) -> [3]` {nbsp} {nbsp} midpoint: stem:[1/2 + 2 ]
...
^a|`T~2~M11~2~([2]) -> [4]`{nbsp}  midpoint: stem:[2 + 2/2 = 3]
^a|`T~3~M11~2~([2]) -> [5]`{nbsp} midpoint: stem:[1/2 + 3]
...
Let _s_ be the midpoint of (p, p + k).
...
image:pcs-256-M11-free-axe.png[] : `T~-3~M11~2~([256])` -> `[7811]`

Others expressions with valid p-value :
`T~-3~M11~2~([256])` = `T~3~M11~5~([256])` = `T~1~M11~6~([256])` -> `[7811]`
