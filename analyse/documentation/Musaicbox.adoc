...
=== Orbit
Orbit is a set where all elements (pcs) share the same equivalence property (see also _setoid_ or _bishop set_)

Example : Orbit cyclic of major chord `{ C,E,G }` is a set with equivalence relations "_having same interval structure (4,3,5)_". This set is composed of 12 elements (`{ C, E, G }, { C#, E#, G# }, ..., { B, D#, F# }`)
...
If we organize the 4096 pcs set into subsets with equivalence relation "_having same intervallic structure_", we obtain 352 subsets (orbits), a partition of 4096 pcs set.
