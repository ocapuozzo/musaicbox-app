...
Two entities are said "equivalent" if they share a same property.

Example of property, as a _proposition_ : "_To be a major triad_"

There are only 12 pcs, among the 4096 pcs, where this proposition is true.

Such proposition can take form of a _equivalence relation_, by example ;
_R_ = "_having same structure xxad_" (as monad, minor dyad, major triadfootnote:[tetrad, pentad and more. See https://en.wikipedia.org/wiki/Numeral_prefix], ...)
...
=== Orbit
Orbit is a set where all elements (pcs) connected by the same equivalence relation.footnote:[see _setoid_ or _bishop set_]

Example : Orbit cyclic of major triad as `{ C,E,G }` is a set with equivalence relations "_having same structure of major triad_". This set is composed of 12 elements (`{ C, E, G }, { C#, E#, G# }, ..., { B, D#, F# }`)
