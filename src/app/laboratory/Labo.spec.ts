...
describe('Laboratory explorer', () => {
  ...
  it('n=12 - Explore all sub-groups of musaic group - up to transposition', () => {
    ...
    for (const e of groupComputed) {
      const orbitsByCard = numberOrbitOrderedByCardinal(e[1].orbits)

      let  stringNumberOrbitByCardinal = numberOrbitOrderedByCardinal(e[1].orbits).join(';')
      for (let i = 0; i <= 12-orbitsByCard.length; i++) {
        stringNumberOrbitByCardinal +=";"
      }

      // console.log(e[0] + "|" + e[1].orbits.length +"|" + numberOrbitByCardinal)
      //group name "n=12 [M1-T0]" => "[M1-T0]"
      // cols : opsGroupeName; [enum by cardinality] ; number orbits ; number ops in group ;  name (empty)
      // TODO get name from service group action ...
