...
export class ManagerPageWBService {
  ...
  addPcs({somePcs, circularAlign, indexCenterElement, templateDto}: {
    ...
    somePcs.forEach(pcs => {
      ...
      pcsDto.uiMusaic.rounded = pcsDto.uiMusaic.rounded || pcs.isComingFromOrbit()
  ...
  private doMakeSetOperation(opName: string) {
    if (opName === 'Complement' && this.getSelectedPcsDtoIndexes().length > 0) {
      ...
      if (pcsRef.isComingFromOrbit()) {
        newPcs = newPcs.getOrMakeInstanceFromOrbitOfGroupActionOf(pcsRef.orbit!.groupAction!)
      }
